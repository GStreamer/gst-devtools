...
#define PAD_PARENT_IS_MUXER(m) \
    (GST_VALIDATE_MONITOR_GET_PARENT(m) ? \
        GST_VALIDATE_ELEMENT_MONITOR_ELEMENT_IS_MUXER ( \
            GST_VALIDATE_MONITOR_GET_PARENT(m)) : \
        FALSE)
    ...
    monitor, GstFlowReturn ret)
{
  ...
  if (aggregated == GST_FLOW_OK || aggregated == GST_FLOW_UNEXPECTED) {
    /* those are acceptable situations */

    if (GST_PAD_IS_FLUSHING (pad) && ret == GST_FLOW_WRONG_STATE) {
    ...
    if (PAD_PARENT_IS_DEMUXER (monitor) && ret == GST_FLOW_UNEXPECTED) {

    ...
    GstElementConfigureFunc configure_function, gchar ** error_message)
{
  ...
  if (!playbin || !videosink || !audiosink) {
    *error_message = g_strdup ("Playbin2 and/or fakesink not available");
#if 0
...
static gboolean
input_selector_data_probe (GstPad * pad, GstBuffer * buffer, gpointer userdata)
{
  ...
  while (!done) {
    switch (gst_iterator_next (iterator, (gpointer *) & pad)) {
      case GST_ITERATOR_OK:
        ...
        bcd->probe_id = gst_pad_add_data_probe (pad,
            (GCallback) input_selector_data_probe, NULL);
        bcd->pad = pad;
        break;
  ...
  while (!done) {
    switch (gst_iterator_next (iterator, (gpointer *) & pad)) {
      case GST_ITERATOR_OK:
        ...
        gst_pad_remove_data_probe (pad, bcd->probe_id);
...
find_next_pad (GstElement * element, GstPad * pad)
{
  ...
  while (!done) {
    switch (gst_iterator_next (iterator, (gpointer *) & tmp)) {
      case GST_ITERATOR_OK:
        if (first == NULL)
...
static int
find_input_selector (GstElement * element, void *userdata)
{
...
check_track_selection (GstValidateMediaInfo * mi, gchar ** error_message)
{
  ...
  input_selector = gst_iterator_find_custom (iterator,
      (GCompareFunc) find_input_selector, NULL);
  gst_iterator_free (iterator);
  if (!input_selector) {
    ...
    GstValidateMediaInfo * extracted)
{
  ...
  if (expected->stream_info
    ...
    gchar *expected_str = gst_caps_to_string (expected->stream_info->caps);
    gchar *extracted_str = gst_caps_to_string (extracted->stream_info->caps);

    g_print ("Media caps changed: '%s' -> '%s'\n", expected_str, extracted_str);
    g_free (expected_str);
    g_free (extracted_str);
    ret = FALSE;
  }
  return ret;
}
